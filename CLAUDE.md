...
* **Compact Non-Blocking Floating Overlays & Legal Toasts Law**:
  - Floating preference overlays, legal notices, and cookie consent banners must NEVER occupy full-width horizontal bars or exceed $70\text{px}$ in height.
  - Enforce compact bottom-right toast positioning (`fixed bottom-3 right-3 sm:bottom-4 sm:right-4 max-w-sm sm:max-w-[420px]`) with responsive padding (`p-3 sm:p-3.5`).
  - Must feature an accessible top-right dismiss button (`✕`), concise disclosure copy ($\le 2$ lines), and compact action buttons (`Customize`, `Essential Only`, `✓ Accept All`) that store preferences in `localStorage` and dismiss immediately without occluding user prompt composers or canvas controllers.
...
35. **Mandatory Downstream Export Surface, Google Slides Studio & Cloud Bridge Zero-Deformity Law**:
    - **Downstream Export Parity Audit Mandate**: Omni 1.1 visual and functional verification MUST audit downstream export surfaces (`Open in Google Slides ↗` In-Browser Studio Modal, `.pptx` PowerPoint compiler, `.docx` Word compiler, and `docs.google.com/viewer` Cloud Bridge) whenever any blueprint or master builder is created or modified.
    - **Dual Icon Extraction Mandate (`node.value` + `style.image`)**: Exporters (`editablePptxCompiler.ts`, `GoogleWorkspaceDirectOpenModal.tsx`) MUST extract vector icons from **both** inline `<svg>` tags in `node.value` **and** `data:image/svg+xml;base64,...` / `utf8` data URIs in `node.style.image` (protecting data URI internal semicolons before splitting style strings).
    - **1:1 Interactive Twin Default & Strict Z-Order Sorting**: In-Browser Google Slides Studio Slide 1 must default to **1:1 Interactive Twin** (100% pixel-accurate visual twin with clickable hotspot nodes and live label editing). Decomposed shape views must sort vertices strictly by `depth` ascending and `area (width * height)` descending so outer containers never occlude child nodes, with zero artificial size inflation (`Math.max(4.2, widthPct)` prohibited) and standalone icon labels positioned cleanly below `36x36` icons.
    - **Cloud Bridge Public HTTPS, HEAD & Content-Length Contract**: `/api/export/cloud-bridge/[filename]/route.ts` must serve clean `.pptx`/`.docx` filename paths with explicit `HEAD` handlers, `Content-Length` headers, and `/tmp` disk persistence, plus automatic localhost-to-Railway public HTTPS syncing so Google Docs Viewer (`docs.google.com/viewer`) renders every presentation without `"No preview available"` errors.

36. **Permanent Retirement of Obsolete `/workspace` Route Law**:
...
37. **Universal Cloud Studio Parity (Google Docs ⇄ Slides) & Direct External Google Tab (`docs.google.com`) Law**:
    - **100% Interactive Decomposed Diagram Parity in Google Docs Studio (`activeMode === 'docs'`)**:
      - `GoogleWorkspaceDirectOpenModal.tsx` MUST NEVER degrade Google Docs Specification Studio (`activeMode === 'docs'`) into a static `<img>` or clutter it with node inventory tables while Google Slides Studio has an interactive vector canvas.
      - Google Docs Studio MUST render ONLY the **100% Editable Decomposed Vector Diagram Canvas (177 Shapes & 89 Azure SVG Icons)** (`docsDiagramViewMode === 'decomposed-shapes'`), paired with the Left Interactive Sidebar (`Live Diagram & Doc Node Editor`) with **ZERO node information tables or component specification matrices**.
...
38. **Mandatory Post-Fix Governance Triad Synchronization Law (`AGENTS.md` + `SKILL.md` + `hooks.json`)**:
    - Across every project and workspace, whenever any bug fix, root-cause remediation, or user feedback correction is implemented in code, the agent MUST immediately update all three governance layers in lockstep within the same commit:
      1. **Markdown Governance Files**: Synchronize `AGENTS.md`, `GEMINI.md`, and `.agents/AGENTS.md` byte-for-byte.
      2. **Domain Skills**: Synchronize `.agents/skills/*/SKILL.md` and `~/.gemini/config/skills/*/SKILL.md`.
      3. **Automated Quality Gates & Hooks**: Add or update assertions in `.agents/hooks.json`, `scripts/verify_export_slides_quality_gate.ts`, and `scripts/guards/gate_governance_doc_sync.mjs` (enforced automatically by `.git/hooks/pre-commit`) so the defect can never recur in any project.

39. **Google Docs / Word (`.docx`) 100% Native Editable DrawingML Vector Diagram (`<wpg:wgp>`) on a Single Widescreen Landscape Page (`Page 1 / 1`) with Zero Node Information Tables Law (`exportDrawioToEditableDocx`)**:
    - Whenever compiling `.docx` documents for Google Docs and Microsoft Word (`exportDrawioToEditableDocx` and `/api/export/cloud-bridge/[filename]/route.ts`), the generated document MUST NEVER include multi-page node information tables, component inventory matrices, data flow matrices, or Draw.io link banners.
    - Every `.docx` generated by `exportDrawioToEditableDocx` MUST strictly contain:
      1. **Single Widescreen Landscape Page (`Page 1 / 1`, `13.33 in x 7.5 in`, `19200 x 10800 DXA`)**: Uses `PageOrientation.LANDSCAPE` with `width: 10800, height: 19200` DXA (which `docx` writes as `<w:pgSz w:w="19200" w:h="10800" w:orient="landscape"/>`), matching 16:9 widescreen architecture canvases so the entire diagram fits cleanly on **Page 1 of 1**.
      2. **100% Native Editable OpenXML DrawingML Wordprocessing Group (`<w:drawing><wp:inline>...<wpg:wgp>`)**: Converts every Draw.io vertex (enclave containers, subnets, and service cards) and edge (polyline waypoints, orthogonal connectors, dashed lines, and `<a:tailEnd type="triangle"/>` arrowheads) into native `<wps:wsp>` vector shapes with `<a:solidFill>`, `<a:ln>`, and `<wps:txbx>` (`<w:txbxContent>`) textboxes.
      3. **Zero Node Tables & Schema Auto-Healing**: Strictly zero `<w:tbl>` tables in `.docx` output, zero component matrices in `GoogleWorkspaceDirectOpenModal.tsx`, and `CURRENT_BRIDGE_SCHEMA_VERSION` must be bumped in `/api/export/cloud-bridge/route.ts` whenever `editableDocxCompiler.ts` changes so cached `.docx` files on Railway automatically self-heal on demand.

40. **Native Word DrawingML 4-Layer Z-Order, Zero-Corruption Transparent Labels (`<a:noFill/>`) & Google Docs Viewer `<wpg:wgp>` Vector Badge Law (`editableDocxCompiler.ts`)**:
    - **Zero-Corruption Transparent Label Law (`<a:noFill/>`)**:
      - In Draw.io, standalone text headers and icon bottom labels use `fillColor=none;strokeColor=none`.
      - `makeWpsShape()` MUST explicitly check `fillHex !== 'none'` and `strokeHex !== 'none'` and emit `<a:noFill/>` and `<a:ln><a:noFill/></a:ln>` with zero internal padding (`lIns="0" rIns="0" tIns="0" bIns="0"`) for transparent nodes. Never fall back to default opaque white (`#FFFFFF`) fill or blue (`#0284C7`) stroke on transparent labels.
    - **Google Docs Viewer `<wpg:wgp>` Image Restriction Law**:
      - `docs.google.com/viewer` rejects any `<wpg:wgp>` Wordprocessing Group containing `r:embed` image relationships (`<a:blipFill>` or `<pic:pic>`) and renders a blank page.
      - Therefore, all standalone icon nodes (`hasSvg: true` or `style.image`) inside `<wpg:wgp>` MUST be compiled as pure OpenXML vector shapes (`<a:prstGeom>` such as `roundRect`, `pentagon`, `diamond`, `hexagon`, `ellipse`) with authentic Azure/Cloud service colors and 2–3 character white bold monograms (`WAF`, `NSG`, `DEF`, `KV`, `OAI`, `SRC`, `AI`, `ORC`, `AGT`, `DB`, `BUS`, `VN`, `SN`, etc. via `getServiceBadgeInfo`), paired with separate transparent `<wps:wsp>` bottom label textboxes.
    - **Strict 4-Layer Back-to-Front Z-Order & Dynamic Neighbor Clamping**:
      - `buildNativeWordDrawingMlDiagram()` MUST emit shapes in strict 4-layer back-to-front Z-order:
        1. **Layer 1 (Opaque Containers & Cards)**: Sorted strictly by area (`width * height` descending) so outer enclaves never cover inner cards.
        2. **Layer 2 (Polyline & Orthogonal Connectors)**: Rendered cleanly over containers with `<a:tailEnd type="triangle"/>` arrowheads.
        3. **Layer 3 (Color-Coded Native Vector Service Icon Badges)**: Pure vector geometric badges positioned above bottom labels.
        4. **Layer 4 (Topmost Transparent Headers & Neighbor-Clamped Icon Labels)**: Joins multi-line subnet headers crossed by vertical connector lines (`API Management Subnet`, `App Service Ingress Subnet`) into single-line crisp white pill shields (`height = 18px`, `fillHex = 'FFFFFF'`) so lines pass behind them without slicing text, and clamps horizontal label widths (`minNeighborDist - 4`) across same-row neighbors so adjacent icon labels never overlap horizontally.

41. **Anti-Static-Spoofing, Layout Archetype vs. Subject Domain Separation & Semantic Prompt-to-Canvas Subject Parity Law**:
    - **Strict Separation of Layout Archetype vs. Subject Domain (Zero Static Spoofing)**:
      - Never confuse a **Visual Layout Archetype** (e.g., `Tiered Architectural Infographic`, `Infographic`, `Swimlane`, `Sequence Diagram`, `C4 Topology`) with a **Static Content Template** (`Charlie Hills / Context + Harness + Loop + Graph`, `NOVACURA Biopharma`, etc.).
      - When a user prompt requests a layout archetype for a **new or distinct subject topic** (e.g., `"Open Knowledge format Infographic"`, `"Healthcare FHIR Infographic"`, `"Zero-Trust Security Infographic"`), the prompt router and diagram generator MUST NEVER spoof or return a static template belonging to an unrelated subject domain (`Context + Harness + Loop + Graph` or `Charlie Hills`).
      - Instead, the engine MUST either invoke a dedicated domain-matched generator (`generateOpenKnowledgeInfographicXml` for Open Knowledge Format) or dynamically synthesize the 4-tier infographic structure (`01`–`04` vertical spine, dashed enclaves, decision gates, and bottom rule/prompt bars) populated 100% with the user's requested subject matter.
