  ...
  try {
    ...
    const systemInstruction = isRefactor
      ...
      : [
          ...
          'For a GCP streaming request, the Balanced Production option MUST contain 14-18 meaningful components and explicitly include: external producer, Cloud Armor, Cloud Load Balancing, a Cloud Run or GKE ingestion runtime, Pub/Sub event topic, separate Pub/Sub subscription, a schema/data-quality decision diamond with valid and invalid outcomes, Dataflow, a distinct Pub/Sub dead-letter topic or subscription with retry/replay flow, Cloud Storage raw archive, BigQuery analytical sink, Identity and Access Management, Cloud Logging, and Cloud Monitoring. The primary flow must remain left-to-right from ingress to topic to subscription to validation/processing to sinks. Policy and observability relationships are cross-cutting and must not be inserted inline in the data path.',
