      ...
      <header className={`sticky top-0 z-30 w-full border-b backdrop-blur-md shrink-0 transition-colors ${
        ...
        <div className="max-w-8xl mx-auto px-4 sm:px-6 md:px-12 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Link
              ...
              className={`p-2 rounded-xl border transition-all ${
                isLight ? 'bg-slate-100 hover:bg-slate-200 border-slate-300 text-slate-700' : 'bg-slate-900 border-panel-border/60 text-slate-400 hover:text-white'
              }`}
            >
              <ArrowLeft className="w-4 h-4" />
            </Link>
            <div className="flex items-center gap-2.5">
              <div className="w-9 h-9 rounded-xl bg-amber-500/20 border border-amber-500/40 text-amber-500 font-black flex items-center justify-center shadow-lg shadow-amber-500/10">
                <ShieldCheck className="w-5 h-5" />
              </div>
              <div>
                <h1 className={`text-base font-black tracking-wide ${isLight ? 'text-slate-900' : 'text-white'}`}>Admin Portal</h1>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="px-3 py-1.5 rounded-full bg-amber-500/10 border border-amber-500/30 text-amber-600 dark:text-amber-300 text-xs font-bold flex items-center gap-1.5">
      ...
      <main className="flex-1 w-full max-w-8xl mx-auto px-4 sm:px-6 md:px-12 py-6 sm:py-8 md:py-10 space-y-6 sm:space-y-8">
        {error ? (
          <div className={`p-8 border rounded-2xl text-center max-w-md mx-auto space-y-3 ${
            ...
            <h3 className={`text-lg font-bold ${isLight ? 'text-rose-900' : 'text-white'}`}>Admin Access Required</h3>
            <p className={`text-xs leading-relaxed ${isLight ? 'text-rose-700' : 'text-slate-400'}`}>Forbidden. Admin access required.</p>
            <Link
              href="/dashboard"
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
              <div className={`rounded-2xl p-5 space-y-2 border ${
                isLight ? 'bg-white border-slate-200 shadow-md' : 'glass-panel border-panel-border/40'
              }`}>
                <div className="flex items-center justify-between text-slate-500 dark:text-slate-400">
                  <span className="text-xs font-bold uppercase tracking-wider">Total Registered Users</span>
                  <Users className="w-4 h-4 text-teal-500" />
                </div>
                <p className={`text-3xl font-black ${isLight ? 'text-slate-900' : 'text-white'}`}>{users.length}</p>
              </div>
              ...
              <div className={`rounded-2xl p-5 space-y-2 border ${
                ...
                <p className={`text-3xl font-black ${isLight ? 'text-slate-900' : 'text-white'}`}>{users.filter((u) => !u.is_super_admin && u.global_role !== 'Super-Admin').length}</p>
              </div>
            </div>

            {/* Users Table Header & Search */}
            <div className={`rounded-2xl overflow-hidden shadow-xl space-y-4 border ${
              isLight ? 'bg-white border-slate-200' : 'glass-panel border-panel-border/60'
            }`}>
              <div className={`p-5 border-b flex flex-col sm:flex-row items-center justify-between gap-4 ${
                isLight ? 'bg-slate-50 border-slate-200' : 'border-panel-border/40 bg-[#090d18]/40'
              }`}>
                <div>
                  <h2 className={`text-sm font-extrabold ${isLight ? 'text-slate-900' : 'text-white'}`}>Global System Users</h2>
                  <p className={`text-xs ${isLight ? 'text-slate-600' : 'text-slate-400'}`}>Manage global system roles, root clearance, and account access</p>
                </div>
                  ...
                  <input
                    ...
                    className={`w-full border rounded-xl pl-10 pr-4 py-2 text-xs outline-none ${
                      isLight
                        ...
                        : 'bg-[#070a13] border-panel-border/70 text-white placeholder:text-slate-600 focus:border-amber-500/50'
                    }`}
                  />
                </div>
              </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-xs">
                  <thead className={`uppercase text-[10px] font-extrabold tracking-wider border-b ${
                    isLight ? 'bg-slate-100 text-slate-600 border-slate-200' : 'bg-[#090d18] text-slate-400 border-panel-border/40'
                  }`}>
                    <tr>
                  ...
                  <tbody className={`divide-y ${isLight ? 'divide-slate-200' : 'divide-panel-border/20'}`}>
                    {loading ? (
                      <tr>
                      ...
                      filteredUsers.map((u) => {
                        ...
                        return (
                          <tr key={u.id} className={`transition-all ${isLight ? 'hover:bg-slate-50' : 'hover:bg-slate-900/40'}`}>
                            <td className={`px-6 py-4 font-bold flex items-center gap-2.5 ${isLight ? 'text-slate-900' : 'text-white'}`}>
                              <div className="w-7 h-7 rounded-lg bg-teal-500/20 text-teal-600 dark:text-teal-400 font-bold flex items-center justify-center text-xs">
                                {(u.name || u.email)[0].toUpperCase()}
                              </div>
                              <span>{u.name || u.email.split('@')[0]}</span>
                            </td>
                            <td className={`px-6 py-4 font-mono ${isLight ? 'text-slate-700' : 'text-slate-300'}`}>{u.email}</td>
                            ...
                            <td className="px-6 py-4 text-right">
                              <select
                                ...
                                className={`border rounded-lg px-2.5 py-1 text-xs outline-none cursor-pointer ${
                                  isLight
                                    ? 'bg-white border-slate-300 text-slate-900 focus:border-amber-500'
                                    : 'bg-[#070a13] border-panel-border/70 text-slate-200 focus:border-amber-500/50'
                                }`}
                              >
