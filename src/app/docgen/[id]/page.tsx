  ...
  const renderExecutiveDocument = (text: string) => {
    ...
    while (i < lines.length) {
      ...
      if (line.trim().startsWith('```')) {
        ...
        elements.push(
          <div key={`code-${i}`} className={`my-5 rounded-2xl border shadow-lg overflow-hidden ${isLight ? 'border-sky-300 bg-white shadow-slate-300/40' : 'border-sky-500/50 bg-slate-950/95 shadow-2xl'}`}>
            <div className={`px-4 py-2.5 border-b flex items-center justify-between ${isLight ? 'bg-slate-100 border-slate-200' : 'bg-slate-900 border-slate-800'}`}>
              <span className={`text-xs font-bold uppercase tracking-wider flex items-center gap-2 ${isLight ? 'text-sky-800' : 'text-sky-300'}`}>
                <FileText className="w-3.5 h-3.5 text-sky-500" /> Architecture Flow Specification
              </span>
            ...
            <pre className={`p-4 font-mono text-xs overflow-x-auto ${isLight ? 'bg-slate-50 text-teal-800' : 'bg-slate-950 text-teal-300'}`}>
              {codeLines.join('\n')}
            </pre>
          </div>
        );
