    ...
    <div className={theme === 'dark' ? 'min-h-screen bg-[#070b16] text-slate-100' : 'min-h-screen bg-slate-50 text-slate-900'}>
      <header className={`sticky top-0 z-30 w-full border-b backdrop-blur ${theme === 'dark' ? 'border-slate-800 bg-[#070b16]/90' : 'border-slate-200 bg-white/90'}`}>
        <div className="mx-auto flex max-w-[1600px] items-center justify-between gap-6 px-6 py-4 md:px-12">
          <Link href="/dashboard" className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-fuchsia-600 to-indigo-600 shadow-lg shadow-fuchsia-500/20"><Sparkles className="h-5 w-5 text-white" /></div><div><p className="text-base font-black tracking-tight">PromptCanvas Studio 3</p><p className="text-xs font-medium text-slate-400">Pure multimodal content generation</p></div></Link>
