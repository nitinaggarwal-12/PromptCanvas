    ...
    (async () => {
      try {
        ...
        if (latestXml) {
          ...
          showToast(`✨ Loaded architecture "${data.name || 'Diagram'}" from Library`);
        }
      } catch (err) {
        console.error('Failed to hydrate diagram in Studio 1:', err);
      }
    })();
        ...
        <div className="fixed inset-0 z-50 flex justify-end bg-black/60 backdrop-blur-xs transition-opacity animate-in fade-in">
          ...
          <div className={`relative z-10 w-full max-w-md h-full shadow-2xl flex flex-col border-l transition-transform ${
            ...
            <div className={`p-3 border-t flex items-center justify-between text-[11px] font-bold ${
              ...
              <Link href="/library?studio=studio1" className="text-purple-500 hover:underline font-black flex items-center gap-1">
                <span>Open Library Page</span>
                <ExternalLink className="w-3 h-3" />
              </Link>
            </div>
          </div>
...
export default function LaunchStudio1Page() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center">Loading Prompt Lab...</div>}>
      <Studio1Content />
    </Suspense>
  );
}
