          ...
          {currentUser?.is_guest && !isGuestDisclaimerDismissed && (
            <div className={`w-full border-b py-2 px-3 sm:px-6 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs md:text-sm backdrop-blur-md z-40 shrink-0 animate-fade-in ${
              ...
              <div className="flex items-center gap-2 font-medium min-w-0">
                <ShieldAlert className={`w-4 h-4 shrink-0 ${canvasTheme === 'light' ? 'text-amber-700' : 'text-amber-400'}`} />
                <span className="truncate sm:whitespace-normal">
                  <strong className={`font-bold ${canvasTheme === 'light' ? 'text-amber-950 font-black' : 'text-amber-300'}`}>Guest Mode Disclaimer:</strong>{' '}
                  <span className={canvasTheme === 'light' ? 'text-amber-900 font-medium' : 'text-amber-200'}>
          ...
          <>
            ...
            <header className={`h-14 border-b flex items-center justify-between px-3 md:px-6 backdrop-blur-md gap-3 relative shrink-0 transition-colors ${tourStep !== null ? 'z-[60]' : 'z-30'} ${
              ...
              <div className="hidden xl:flex items-center gap-2 shrink-0">
                {isAnyAIBusy && (
                  <div className={`flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold animate-pulse border ${
                    canvasTheme === 'light'
              ...
              {!activeDiagram ? (
                ...
                <div className="w-full h-full flex flex-col relative overflow-hidden">
                  ...
                  {previewVersion && previewVersion.id !== activeVersion?.id && (
                    <div className={`w-full border-b px-4 py-2.5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2.5 text-xs font-semibold backdrop-blur-md z-30 shrink-0 animate-fade-in shadow-md ${
                      canvasTheme === 'light'
                        ? 'bg-amber-50/95 border-amber-300 text-amber-950'
                        : 'bg-amber-500/15 border-amber-500/40 text-amber-200'
                    }`}>
                      <div className="flex items-center gap-2 min-w-0">
                        <AlertTriangle className={`w-4 h-4 shrink-0 ${canvasTheme === 'light' ? 'text-amber-700' : 'text-amber-400'}`} />
                        <span className="truncate">
                          Viewing Historical Snapshot <b className={canvasTheme === 'light' ? 'text-slate-900' : 'text-white'}>v{previewVersion.version_number}</b> (Read-Only) — Submitting a prompt will fork this into a new version <b className={canvasTheme === 'light' ? 'text-teal-700 font-bold' : 'text-teal-300'}>v{(activeDiagram?.versions?.length || 0) + 1}</b>.
                        </span>
                      </div>
                      <div className="flex items-center gap-2 shrink-0 self-end sm:self-auto">
                        <button
                          ...
                          className={`px-2.5 py-1 rounded-lg border text-xs font-bold cursor-pointer transition ${
                            canvasTheme === 'light'
