...
export function injectUseCaseFlavor(xml: string, useCaseTitle: string, userPrompt?: string): string {
  ...
  updatedXml = updatedXml
    ...
    .replace(/Enterprise Document Lake/gi, `[1] ${topicClean} Document &amp; Telemetry Lake`)
    .replace(/Order &amp;amp; Checkout Microservices Pods/gi, `[1.1] ${topicClean} Ingress Pods`)
    .replace(/Order &amp; Checkout Microservices Pods/gi, `[1.1] ${topicClean} Ingress Pods`)
    .replace(/Production Operational OLTP Core/gi, `[1.1] ${topicClean} Operational DB Core`)
    .replace(/Ingress &amp;amp; Perimeter/gi, `${topicClean} Ingress &amp;amp; Perimeter`)
    .replace(/Ingress &amp; Perimeter/gi, `${topicClean} Ingress &amp; Perimeter`)
