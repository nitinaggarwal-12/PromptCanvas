...
export function injectUseCaseFlavor(xml: string, useCaseTitle: string, userPrompt?: string): string {
  ...
  if (isEarlyDiscoveryPrompt) {
    updatedXml = updatedXml
      ...
      // ERD Specific Replacements (Domain Specific for Early Discovery & Target Identification)
      ...
      .replace(/Dim_Physician/g, 'Dim_Assay_Experiment')
      .replace(/Physician Key/g, 'Assay ID (PK)')
      .replace(/Specialization/g, 'Assay Type (Binding/IC50)')
      .replace(/Affiliation/g, 'Lab Facility &amp; Platform')
      .replace(/Ranking/g, 'Assay Protocol Version')
      .replace(/Location/g, 'Lab Location')
      .replace(/Dim_Payer/g, 'Dim_Literature_Corpus')
      .replace(/Payer Key/g, 'Doc DOI / Patent ID (PK)')
      .replace(/Payer Name/g, 'Publisher / Journal')
      .replace(/Type/g, 'Doc Type (Paper/Patent)')
      .replace(/Tier/g, 'Deep Research Impact Tier')
      ...
      .replace(/Fact_Patient_Encounters/g, 'Fact_Virtual_Synthesis_Routes')
      .replace(/Encounter ID/g, 'Synthesis Route ID')
      .replace(/Diagnosis ID/g, 'Reaction Steps Count')
      .replace(/Physician ID/g, 'Assay ID')
