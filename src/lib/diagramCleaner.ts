...
export function injectUseCaseFlavor(xml: string, useCaseTitle: string, userPrompt?: string): string {
  ...
  updatedXml = updatedXml
    ...
    .replace(/Enterprise Document Lake/gi, `[1] ${shortBrand} Document &amp; Telemetry Lake`)
    .replace(/Order &amp;amp; Checkout Microservices Pods/gi, `[1.1] ${shortBrand} Ingress Pods`)
    .replace(/Order &amp; Checkout Microservices Pods/gi, `[1.1] ${shortBrand} Ingress Pods`)
    .replace(/Production Operational OLTP Core/gi, `[1.1] ${shortBrand} DB Core`)
    .replace(/Ingress &amp;amp; Perimeter/gi, `${shortBrand} Ingress &amp;amp; Perimeter`)
    .replace(/Ingress &amp; Perimeter/gi, `${shortBrand} Ingress &amp; Perimeter`)
    .replace(/IoT Devices &amp; App Telemetry/gi, `IoT &amp; ${shortBrand} Telemetry`)
    .replace(/Route 53 DNS &amp; AWS WAF/gi, `Route 53 (${shortBrand} Ingress)`)
    .replace(/Batch &amp; Streaming Sources/gi, `Batch &amp; ${shortBrand} Streams`)
    .replace(/Cloud DNS &amp; Cloud CDN/gi, `Cloud DNS (${shortBrand} Edge)`)
    .replace(/Polyrepo Git Commits/gi, `${shortBrand} Git Commits`)
    .replace(/Cloud Run UI \(Private App Subnet\)/gi, `Cloud Run (${shortBrand} UI)`)
    .replace(/Cloud Run API Microservices/gi, `Cloud Run (${shortBrand} API)`)
    .replace(/Cloud SQL HA PostgreSQL/gi, `Cloud SQL (${shortBrand} DB)`)
    .replace(/Static Media Lake &amp; Private Assets/gi, `${shortBrand} Storage Lake`)
    .replace(/MQTT \/ HTTPS Edge Ingestion Gateway/gi, `[1] ${shortBrand} Edge Ingestion`)
    .replace(/Pub\/Sub High-Throughput Ingestion Topics/gi, `[2] ${shortBrand} Pub/Sub Topics`)
    .replace(/Frontend Envoy Mesh Pods/gi, `[1] ${shortBrand} Ingress Pods`)
    .replace(/Core API Business Logic Pods/gi, `[2] ${shortBrand} Core API Pods`)
    .replace(/Bronze Tier Raw Data Bucket/gi, `[3] Bronze (${shortBrand} Raw)`)
    .replace(/Gold Tier Curated Delta Lake/gi, `[5] Gold (${shortBrand} Metrics)`)
    .replace(/Vertex AI Matching Engine/gi, `[3] Vertex AI (${shortBrand} Vectors)`)
    .replace(/Gemini 1\.5 Pro Reasoning Agent/gi, `[4] Gemini (${shortBrand} AI Core)`)
    .replace(/Primary Application Cluster/gi, `[8] ${shortBrand} Cluster`)
    .replace(/Source Code Repository/gi, `[1] ${shortBrand} Polyrepo`)
    .replace(/Polyrepo Git Source/gi, `[1] ${shortBrand} Polyrepo`)
    .replace(/PostgreSQL \/ MySQL OLTP Core/gi, `[1] ${shortBrand} OLTP DB`)
    .replace(/Order &amp; Checkout Microservice/gi, `[1] ${shortBrand} Microservice`)
    .replace(/Multi-Modal Ingress &amp; Routing Gateway/gi, `[1] ${shortBrand} Gateway`)
    .replace(/FastAPI REST API Microservice/gi, `${shortBrand} API Service`)
    .replace(/Next\.js Frontend Web Application/gi, `${shortBrand} Web Portal`)
    .replace(/Cloud Run \(FastAPI Backend API\)/gi, `Cloud Run (${shortBrand} API)`)
    .replace(/Cloud Run \(Frontend SSR Microservice\)/gi, `Cloud Run (${shortBrand} UI)`)
    .replace(/Cloud SQL PostgreSQL 15/gi, `Cloud SQL (${shortBrand} DB)`)
  ...
  updatedXml = updatedXml
    ...
    .replace(/Google Cloud Project \(ITACS Platform Production\)/g, `${topicClean} Production Cloud Architecture`)
    .replace(/AWS Serverless Event-Driven Microservices/g, `${topicClean} - Event-Driven Microservices`)
    .replace(/GCP Multi-Region Active-Passive Disaster Recovery/g, `${topicClean} - Multi-Region DR Topology`)
    .replace(/GCP AI Cognitive Architecture \(Retrieval-Augmented Generation \/ RAG\)/g, `${topicClean} - AI Agentic RAG Architecture`)
    .replace(/AWS Zero-Trust Secure VPC Network Infrastructure/g, `${topicClean} - Zero-Trust Secure VPC Network`)
    .replace(/GCP Industrial IoT Telemetry Ingestion &amp; Analytics/g, `${topicClean} - Industrial IoT Ingestion &amp; Analytics`)
    .replace(/Enterprise DevSecOps Polyrepo CI\/CD Pipeline Architecture/g, `${topicClean} - DevSecOps Polyrepo CI/CD Pipeline`)
    .replace(/UNIFIED GOVERNANCE &amp; STATE-MACHINE LIFECYCLE/g, `${shortBrandUpper} UNIFIED GOVERNANCE &amp; STATE-MACHINE LIFECYCLE`)
    .replace(/TOTAL UNIFIED SYSTEM VIEW: Data, Cognition, Deployment, (?:&amp;|&amp;amp;) Governance/g, `${shortBrandUpper} TOTAL UNIFIED SYSTEM VIEW`)
    .replace(/COMPLETE END-TO-END DYNAMIC SEQUENCE DIAGRAM/g, `${shortBrandUpper} COMPLETE END-TO-END DYNAMIC SEQUENCE DIAGRAM`)
    .replace(/ITACS Governing Cloud Tenant/g, `${shortBrandUpper} Governing Cloud Tenant`)
    ...
    .replace(/Review Drug Launch Strategy/gi, `Review ${topic} Operational Strategy`)
