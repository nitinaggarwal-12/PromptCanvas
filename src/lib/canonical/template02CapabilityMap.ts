...
export function generateTemplate02CapabilityMapXml(domainFlavor = "biopharma", theme: "light" | "dark" = "light"): string {
  ...
  text("exp_title", "USER / BUSINESS EXPERIENCE LAYER", 534, 70, 360, 18, "fontSize=11.5;fontStyle=1;fontColor=#6D28D9;align=center;");
  ...
  expCards.forEach((ec, i) => {
    ...
    const html = "<table style=\"width:100%;height:100%;border-collapse:collapse;\">" +
      ...
      "</tr>" +
    "</table>";
    c.push("<mxCell id=\"exp_c_" + i + "\" value=\"" + E(html) + "\" style=\"rounded=1;whiteSpace=wrap;html=1;fillColor=#FFFFFF;strokeColor=#DDD6FE;strokeWidth=1;\" vertex=\"1\" parent=\"1\"><mxGeometry x=\"" + x + "\" y=\"90\" width=\"230\" height=\"60\" as=\"geometry\"/></mxCell>");
  });

  // 3. LEFT COLUMN: BUSINESS OUTCOMES (x: 24, y: 68, w: 186, h: 510)
  rect("outcomes_container", "", 24, 68, 186, 510, "strokeColor=#3B82F6;strokeWidth=1.5;fillColor=#EFF6FF;shadow=0;");
  ...
  "</tr></table>";
  text("outcomes_hdr", outcomeHeaderHtml, 28, 74, 178, 22, "align=center;");
  ...
  const outcomes = [
    ...
    { title: "Improved trial execution", desc: "Optimize trials with better planning, monitoring and data.", icon: "https://api.iconify.design/lucide:clipboard-check.svg?color=%231D4ED8" },
    ...
    { title: "Reusable knowledge", desc: "Capture, standardize and reuse institutional knowledge.", icon: "https://api.iconify.design/lucide:book-open.svg?color=%231D4ED8" },
  ...
  outcomes.forEach((o, i) => {
    ...
    const html = "<table style=\"width:100%;height:100%;border-collapse:collapse;\">" +
      "<tr>" +
        "<td style=\"width:30px;vertical-align:top;padding-top:3px;text-align:center;\">" +
          "<img src=\"" + o.icon + "\" width=\"17\" height=\"17\" style=\"display:block;margin:0 auto;\"/>" +
        "</td>" +
        "<td style=\"vertical-align:top;padding-left:4px;padding-top:1px;text-align:left;\">" +
          "<div style=\"font-size:9.5px;font-weight:800;color:#1E3A8A;line-height:1.2;\">" + o.title + "</div>" +
          "<div style=\"font-size:7.5px;color:#334155;margin-top:1px;line-height:1.15;\">" + o.desc + "</div>" +
        "</td>" +
      "</tr>" +
    "</table>";
    c.push("<mxCell id=\"out_c_" + i + "\" value=\"" + E(html) + "\" style=\"rounded=1;whiteSpace=wrap;html=1;fillColor=#FFFFFF;strokeColor=#BFDBFE;strokeWidth=1.2;\" vertex=\"1\" parent=\"1\"><mxGeometry x=\"30\" y=\"" + y + "\" width=\"174\" height=\"72\" as=\"geometry\"/></mxCell>");
  });

  // 4. RIGHT COLUMN: PRIMARY PERSONAS (x: 1220, y: 68, w: 196, h: 510)
  rect("personas_container", "", 1220, 68, 196, 510, "strokeColor=#16A34A;strokeWidth=1.5;fillColor=#F0FDF4;shadow=0;");
  ...
  "</tr></table>";
  text("personas_hdr", personaHeaderHtml, 1224, 74, 188, 22, "align=center;");
  ...
  personas.forEach((p, i) => {
    ...
    const html = "<table style=\"width:100%;height:100%;border-collapse:collapse;\">" +
      "<tr>" +
        "<td style=\"width:28px;vertical-align:middle;text-align:center;\">" +
          "<img src=\"" + p.icon + "\" width=\"17\" height=\"17\" style=\"display:block;margin:0 auto;\"/>" +
        "</td>" +
        "<td style=\"vertical-align:middle;padding-left:4px;text-align:left;\">" +
          "<div style=\"font-size:9.5px;font-weight:800;color:#166534;\">" + p.title + "</div>" +
        "</td>" +
      "</tr>" +
    "</table>";
    c.push("<mxCell id=\"per_c_" + i + "\" value=\"" + E(html) + "\" style=\"rounded=1;whiteSpace=wrap;html=1;fillColor=#FFFFFF;strokeColor=#86EFAC;strokeWidth=1.2;\" vertex=\"1\" parent=\"1\"><mxGeometry x=\"1228\" y=\"" + y + "\" width=\"180\" height=\"52\" as=\"geometry\"/></mxCell>");
  });

  // 5. CENTER PLATFORM: NOVACURA Bio-Pharma Platform (x: 224, y: 168, w: 980, h: 410)
  rect("main_platform", "", 224, 168, 980, 410, "strokeColor=#1D4ED8;strokeWidth=2;fillColor=#FFFFFF;shadow=0;");

  const logoCenterHtml = "<table style=\"margin:0 auto;\"><tr>" +
    "<td style=\"vertical-align:middle;padding-right:10px;\">" +
      "<svg width=\"30\" height=\"22\" viewBox=\"0 0 40 32\"><path d=\"M4 28L14 4h9L15 28z\" fill=\"#2563EB\"/><path d=\"M18 28L28 4h9L27 28z\" fill=\"#1E3A8A\"/></svg>" +
    "</td>" +
    "<td style=\"vertical-align:middle;text-align:left;\">" +
      "<div style=\"font-size:17px;font-weight:900;color:#1E3A8A;letter-spacing:0.8px;line-height:1;\">NOVACURA Bio-Pharma Platform</div>" +
      "<div style=\"font-size:9px;font-weight:800;color:#2563EB;line-height:1.2;letter-spacing:1px;margin-top:2px;\">CORE BUSINESS CAPABILITIES</div>" +
    "</td>" +
  "</tr></table>";
  text("novacura_center_hdr", logoCenterHtml, 464, 172, 500, 30, "align=center;");
  ...
  const domainBox = (id: string, num: string, title: string, domainIcon: string, items: { name: string; icon: string; ai?: boolean }[], x: number, y: number, w: number, h: number, border = "#BFDBFE", bg = "#FFFFFF", titleColor = "#1E3A8A", badgeBg = "#1D4ED8") => {
    ...
    items.forEach(it => {
      ...
      itemsHtml += "<div style=\"display:flex;align-items:center;background:" + itBg + ";border:1px solid " + itBorder + ";border-radius:4px;padding:2px 4px;margin-bottom:2px;box-shadow:0 1px 2px rgba(0,0,0,0.03);\">" +
        "<img src=\"" + it.icon + "\" width=\"12\" height=\"12\" style=\"margin-right:5px;flex-shrink:0;\"/>" +
        "<div style=\"font-size:8.5px;font-weight:" + (it.ai ? "800" : "600") + ";color:" + itColor + ";white-space:nowrap;overflow:hidden;text-overflow:ellipsis;\">" + it.name + "</div>" +
      "</div>";
    });

    const cardHtml = "<div style=\"padding:2px 3px;\">" +
      "<div style=\"display:flex;align-items:center;margin-bottom:3px;\">" +
        "<div style=\"width:15px;height:15px;border-radius:50%;background:" + badgeBg + ";color:#FFF;font-size:8.5px;font-weight:900;display:flex;align-items:center;justify-content:center;margin-right:4px;flex-shrink:0;\">" + num + "</div>" +
        "<img src=\"" + domainIcon + "\" width=\"14\" height=\"14\" style=\"margin-right:4px;flex-shrink:0;\"/>" +
        "<div style=\"font-size:9.5px;font-weight:800;color:" + titleColor + ";\">" + title + "</div>" +
      "</div>" +
      itemsHtml +
    "</div>";
  ...
  // Row 1 of Domains (Top 4 Domains, y: 206, h: 162, w: 230)
  ...
  ], 236, 206, 230, 162, "#BFDBFE", "#FFFFFF", "#1E3A8A", "#1D4ED8");
  ...
  ], 478, 206, 230, 162, "#BFDBFE", "#FFFFFF", "#1E3A8A", "#1D4ED8");
  ...
  ], 720, 206, 230, 162, "#BFDBFE", "#FFFFFF", "#1E3A8A", "#1D4ED8");
  ...
  ], 962, 206, 230, 162, "#DDD6FE", "#FFFFFF", "#4C1D95", "#6D28D9");

  // Row 2 of Domains (Bottom 4 Domains, y: 376, h: 194, w: 230)
  ...
  ], 236, 376, 230, 194, "#99F6E4", "#FFFFFF", "#0F766E", "#0D9488");
  ...
  ], 478, 376, 230, 194, "#99F6E4", "#FFFFFF", "#0F766E", "#0D9488");
  ...
  ], 720, 376, 230, 194, "#DDD6FE", "#FFFFFF", "#581C87", "#7C3AED");
  ...
  ], 962, 376, 230, 194, "#CBD5E1", "#FFFFFF", "#1E293B", "#334155");

  // Middle layer connecting edges
  edge("e_exp_to_main", "exp_layer", "main_platform", "", "#8B5CF6", false, "exitX=0.5;exitY=1;entryX=0.5;entryY=0;startArrow=block;startFill=1;");
  edge("e_outcomes_to_main", "outcomes_container", "main_platform", "", "#2563EB", false, "exitX=1;exitY=0.5;entryX=0;entryY=0.5;");
  edge("e_personas_to_main", "personas_container", "main_platform", "", "#16A34A", false, "exitX=0;exitY=0.5;entryX=1;entryY=0.5;startArrow=block;startFill=1;");

  // 6. BOTTOM CONTAINER: SHARED DIGITAL FOUNDATION (x: 24, y: 590, w: 1392, h: 90)
  rect("foundation_container", "", 24, 590, 1392, 90, "strokeColor=#2563EB;strokeWidth=1.5;fillColor=#F8FAFC;shadow=0;");
  text("found_title", "SHARED DIGITAL FOUNDATION", 560, 592, 320, 18, "fontSize=11.5;fontStyle=1;fontColor=#1E40AF;align=center;");
    ...
    const html = "<table style=\"width:100%;height:100%;border-collapse:collapse;\">" +
      "<tr>" +
        "<td style=\"width:34px;vertical-align:middle;text-align:center;\">" +
          "<div style=\"width:30px;height:30px;border-radius:6px;background:#EFF6FF;border:1px solid #BFDBFE;display:flex;align-items:center;justify-content:center;margin:0 auto;\">" +
            "<img src=\"" + fd.icon + "\" width=\"16\" height=\"16\" style=\"display:block;margin:auto;\"/>" +
          "</div>" +
        "</td>" +
        "<td style=\"vertical-align:middle;padding-left:6px;text-align:left;\">" +
          "<div style=\"font-size:9.5px;font-weight:800;color:#1E3A8A;line-height:1.2;\">" + fd.title + "</div>" +
          "<div style=\"font-size:7.5px;color:#334155;margin-top:1px;line-height:1.15;\">" + fd.desc + "</div>" +
        "</td>" +
      "</tr>" +
    "</table>";
    c.push("<mxCell id=\"fd_c_" + i + "\" value=\"" + E(html) + "\" style=\"rounded=1;whiteSpace=wrap;html=1;fillColor=#FFFFFF;strokeColor=#BFDBFE;strokeWidth=1;\" vertex=\"1\" parent=\"1\"><mxGeometry x=\"" + x + "\" y=\"612\" width=\"264\" height=\"58\" as=\"geometry\"/></mxCell>");
  });

  edge("e_main_to_found", "main_platform", "foundation_container", "", "#2563EB", false, "exitX=0.5;exitY=1;entryX=0.5;entryY=0;startArrow=block;startFill=1;");

  // 7. LEGEND BOX (x: 24, y: 692, w: 1392, h: 42)
  rect("legend_box", "", 24, 692, 1392, 42, "strokeColor=#CBD5E1;strokeWidth=1.2;fillColor=#F8FAFC;");
  
  const legendHtml = "<table style=\"width:100%;height:100%;text-align:left;margin-left:12px;\"><tr>" +
    "<td style=\"width:80px;vertical-align:middle;font-size:10.5px;font-weight:900;color:#0F172A;\">LEGEND</td>" +
