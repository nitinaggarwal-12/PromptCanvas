...
export function generateTemplate27ThreatModelXml(
  ...
  cell("hdr_num", "27", 16, 12, 68, 54, "shape=rectangle;rounded=1;arcSize=14;fillColor=#6D28D9;strokeColor=#6D28D9;fontColor=#FFFFFF;fontSize=32;fontStyle=1;align=center;verticalAlign=middle;");
  
  cell(
    "hdr_title",
    `<div style='font-size:24px;font-weight:900;color:#0F172A;letter-spacing:0.5px;'>THREAT MODEL ARCHITECTURE</div>` +
  ...
  cell("lbl_trust_zones", "SYSTEM CONTEXT &amp; DATA FLOW (TRUST ZONES)", 16, 80, 1074, 16, "text;html=1;strokeColor=none;fillColor=none;fontColor=#1E40AF;fontSize=8.5;fontStyle=1;align=center;verticalAlign=middle;");
  ...
  cell("lbl_shared_sec", "SHARED SECURITY SERVICES (ACROSS ALL ZONES)", 24, 218, 1056, 12, "text;html=1;strokeColor=none;fillColor=none;fontColor=#7C3AED;fontSize=6.5;fontStyle=1;align=center;verticalAlign=middle;");
  ...
  sharedSecItems.forEach((ss, idx) => {
    ...
    cell(`ss_${idx}`, `<div style="display:flex;align-items:center;gap:3px;"><span style="font-size:10px;">${ss.icon}</span><span style="font-size:5.5px;font-weight:800;color:#0F172A;line-height:1.1;">${ss.t}</span></div>`, ssx, 230, 142, 22, "rounded=1;arcSize=4;fillColor=#FFFFFF;strokeColor=#CBD5E1;html=1;align=left;verticalAlign=middle;padding=2;");
  });
  ...
  cell("lbl_attack_surf", "ATTACK SURFACE MAP", 16, 270, 1074, 12, "text;html=1;strokeColor=none;fillColor=none;fontColor=#DC2626;fontSize=7.5;fontStyle=1;align=center;verticalAlign=middle;");
  ...
  attackSurfaces.forEach((as, idx) => {
    const asx = 22 + idx * 96;
    const pillsHtml = as.tags.map(tg => `<span style="background:#1E3A8A;color:#FFFFFF;padding:0.5px 3px;border-radius:4px;font-size:5px;font-weight:900;margin-right:1px;">${tg}</span>`).join("");
    cell(`as_${idx}`, `<div style="text-align:center;"><span style="font-size:11px;">${as.icon}</span><div style="font-size:5px;font-weight:800;color:#0F172A;line-height:1.1;margin-top:1px;">${as.t}</div><div style="margin-top:2px;">${pillsHtml}</div></div>`, asx, 284, 92, 54, "rounded=1;arcSize=4;fillColor=#F8FAFC;strokeColor=#E2E8F0;html=1;align=center;verticalAlign=middle;padding=1;");
  });
  ...
  cell("lbl_scenarios", "THREAT SCENARIOS &amp; MITIGATIONS (EXAMPLES)", 16, 350, 1074, 14, "text;html=1;strokeColor=none;fillColor=none;fontColor=#1E40AF;fontSize=7.5;fontStyle=1;align=center;verticalAlign=middle;");

  // Headers
  cell("th_scen", "THREAT SCENARIO", 20, 366, 120, 16, "fillColor=#EFF6FF;fontColor=#1E40AF;fontSize=6;fontStyle=1;strokeColor=#BFDBFE;align=center;verticalAlign=middle;");
  cell("th_desc", "DESCRIPTION", 142, 366, 170, 16, "fillColor=#EFF6FF;fontColor=#1E40AF;fontSize=6;fontStyle=1;strokeColor=#BFDBFE;align=center;verticalAlign=middle;");
  cell("th_imp", "POTENTIAL IMPACT", 314, 366, 160, 16, "fillColor=#EFF6FF;fontColor=#1E40AF;fontSize=6;fontStyle=1;strokeColor=#BFDBFE;align=center;verticalAlign=middle;");
  cell("th_mit", "MITIGATIONS / CONTROLS", 476, 366, 290, 16, "fillColor=#EFF6FF;fontColor=#1E40AF;fontSize=6;fontStyle=1;strokeColor=#BFDBFE;align=center;verticalAlign=middle;");
  cell("th_det", "DETECTION &amp; RESPONSE", 768, 366, 316, 16, "fillColor=#EFF6FF;fontColor=#1E40AF;fontSize=6;fontStyle=1;strokeColor=#BFDBFE;align=center;verticalAlign=middle;");

  const scenRows = [
    {
      name: "Prompt Injection", bg: "#6D28D9",
      desc: "Attacker crafts input to override instructions and exfiltrate data.",
      impact: "Data leakage, harmful responses", risk: "Risk: High", rfg: "#DC2626",
    ...
    },
    {
      ...
      impact: "PII/PHI exposure, compliance breach", risk: "Risk: High", rfg: "#DC2626",
    ...
    },
    {
      ...
      impact: "Incorrect / biased answers, reputational damage", risk: "Risk: High", rfg: "#DC2626",
    ...
    },
    {
      ...
      impact: "Service disruption, SLA impact", risk: "Risk: Medium", rfg: "#D97706",
    ...
    },
    {
      ...
      impact: "Full system access, data compromise", risk: "Risk: High", rfg: "#DC2626",
    ...
    },
    {
      ...
      impact: "Data / service impact, trust erosion", risk: "Risk: Medium", rfg: "#D97706",
  ...
  scenRows.forEach((sr, idx) => {
    ...
    cell(`sr_n_${idx}`, sr.name, 20, sry, 120, 31, `shape=rectangle;rounded=1;arcSize=4;fillColor=${sr.bg};fontColor=#FFFFFF;fontSize=5.5;fontStyle=1;align=center;verticalAlign=middle;`);
    cell(`sr_d_${idx}`, `<div style="font-size:5px;line-height:1.15;color:#0F172A;padding:2px;">${sr.desc}</div>`, 142, sry, 170, 31, "html=1;strokeColor=#E2E8F0;fillColor=#FFFFFF;align=left;verticalAlign=middle;");
    cell(`sr_i_${idx}`, `<div style="font-size:5px;line-height:1.1;color:#0F172A;padding:2px;"><b>${sr.impact}</b><br/><span style="color:${sr.rfg};font-weight:800;">${sr.risk}</span></div>`, 314, sry, 160, 31, "html=1;strokeColor=#E2E8F0;fillColor=#FFFFFF;align=left;verticalAlign=middle;");
    cell(`sr_m_${idx}`, `<div style="font-size:5px;line-height:1.1;color:#0F172A;padding:2px;">${sr.mit}</div>`, 476, sry, 290, 31, "html=1;strokeColor=#E2E8F0;fillColor=#FFFFFF;align=left;verticalAlign=middle;");
    cell(`sr_dt_${idx}`, `<div style="font-size:5px;line-height:1.1;color:#0F172A;padding:2px;">${sr.det}</div>`, 768, sry, 316, 31, "html=1;strokeColor=#E2E8F0;fillColor=#FFFFFF;align=left;verticalAlign=middle;");
  });
  ...
  cell("lbl_stride_cat", "THREAT CATALOG (STRIDE)", 1100, 80, 420, 14, "text;html=1;strokeColor=none;fillColor=none;fontColor=#7C3AED;fontSize=7.5;fontStyle=1;align=center;verticalAlign=middle;");

  // STRIDE Table Header
  cell("sth_id", "ID", 1104, 96, 26, 14, "fillColor=#FAF5FF;fontColor=#7C3AED;fontSize=5.5;fontStyle=1;strokeColor=#E9D5FF;align=center;verticalAlign=middle;");
  cell("sth_str", "STRIDE", 1132, 96, 68, 14, "fillColor=#FAF5FF;fontColor=#7C3AED;fontSize=5.5;fontStyle=1;strokeColor=#E9D5FF;align=center;verticalAlign=middle;");
  cell("sth_ex", "THREAT EXAMPLES", 1202, 96, 250, 14, "fillColor=#FAF5FF;fontColor=#7C3AED;fontSize=5.5;fontStyle=1;strokeColor=#E9D5FF;align=center;verticalAlign=middle;");
  cell("sth_aff", "AFFECTED", 1454, 96, 62, 14, "fillColor=#FAF5FF;fontColor=#7C3AED;fontSize=5.5;fontStyle=1;strokeColor=#E9D5FF;align=center;verticalAlign=middle;");
  ...
  const rRatingHtml = `<div style="font-size:6px;color:#0F172A;display:flex;justify-content:space-around;align-items:center;height:100%;">
    <b>RISK RATING:</b> &nbsp;
  ...
  cell("lbl_r_sec_ctrls", "KEY SECURITY CONTROLS", 1100, 350, 206, 14, "text;html=1;strokeColor=none;fillColor=none;fontColor=#1E40AF;fontSize=6.5;fontStyle=1;align=center;verticalAlign=middle;");
  const secCtrlsHtml = `<div style="font-size:5.5px;line-height:1.45;color:#0F172A;padding:2px 6px;">
  ...
  cell("lbl_r_comp_std", "COMPLIANCE &amp; STANDARDS", 1314, 350, 206, 14, "text;html=1;strokeColor=none;fillColor=none;fontColor=#166534;fontSize=6.5;fontStyle=1;align=center;verticalAlign=middle;");
  const compStdHtml = `<div style="font-size:5.5px;line-height:1.45;color:#0F172A;padding:2px 6px;">
  ...
  cell("lbl_b_ir", "INCIDENT RESPONSE FLOW (HIGH LEVEL)", 16, 600, 430, 20, "shape=rectangle;rounded=1;arcSize=8;fillColor=#EFF6FF;strokeColor=#CBD5E1;fontColor=#1E40AF;fontSize=8;fontStyle=1;align=center;verticalAlign=middle;");
  ...
  const irDetailsHtml = `<div style="font-size:6.5px;line-height:1.5;color:#0F172A;padding:6px 12px;margin-top:90px;">
    ✔ <b>Automated Isolation:</b> Workloads flagged as compromised are cordoned automatically.<br/>
    ✔ <b>Forensic Preservation:</b> Snapshots &amp; audit trails exported to immutable GCS vault.<br/>
  ...
  cell("lbl_b_mon", "MONITORING &amp; DETECTION", 454, 600, 340, 20, "shape=rectangle;rounded=1;arcSize=8;fillColor=#FAF5FF;strokeColor=#CBD5E1;fontColor=#7C3AED;fontSize=8;fontStyle=1;align=center;verticalAlign=middle;");
  ...
  monIcons.forEach((mi, idx) => {
    ...
    const my = 630 + row * 92;
    cell(`mi_${idx}`, `<div style="font-size:16px;text-align:center;">${mi.icon}</div><div style="font-size:6px;font-weight:800;color:#0F172A;text-align:center;line-height:1.1;margin-top:2px;">${mi.t}</div>`, mx, my, 150, 80, "rounded=1;arcSize=4;fillColor=#F8FAFC;strokeColor=#E2E8F0;html=1;align=center;verticalAlign=middle;padding=2;");
  });

  const monFootHtml = `<div style="font-size:6.5px;line-height:1.45;color:#0F172A;padding:4px 8px;margin-top:190px;">
    • 24/7 Security Operations Center (SOC) coverage.<br/>
    • Automated correlation of cloud audit logs and API anomalies.
  </div>`;
  ...
  cell("lbl_b_risk_sum", "RISK ASSESSMENT SUMMARY", 802, 600, 350, 20, "shape=rectangle;rounded=1;arcSize=8;fillColor=#FFFBEB;strokeColor=#CBD5E1;fontColor=#D97706;fontSize=8;fontStyle=1;align=center;verticalAlign=middle;");

  const riskSumHtml = `<div style="display:flex;align-items:center;justify-content:center;gap:16px;padding:12px;">
    <div style="font-size:48px;">🍩</div>
    <div style="font-size:7px;line-height:1.8;color:#0F172A;">
  ...
  <div style="font-size:6px;line-height:1.4;color:#64748B;padding:6px 12px;">
    Based on CVSS v3.1 and STRIDE severity scoring across all mapped attack surfaces.
  </div>`;
  ...
  cell("lbl_b_notes", "NOTES &amp; GOVERNANCE", 1160, 600, 360, 20, "shape=rectangle;rounded=1;arcSize=8;fillColor=#F8FAFC;strokeColor=#CBD5E1;fontColor=#1E3A8A;fontSize=8;fontStyle=1;align=center;verticalAlign=middle;");
  const notesSecHtml = `<div style="font-size:6.5px;line-height:1.55;color:#0F172A;padding:6px 12px;">
