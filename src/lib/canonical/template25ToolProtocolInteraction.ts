...
export function generateTemplate25ToolProtocolInteractionXml(
  ...
  cell("hdr_num", "25", 16, 12, 68, 54, "shape=rectangle;rounded=1;arcSize=14;fillColor=#6D28D9;strokeColor=#6D28D9;fontColor=#FFFFFF;fontSize=82;fontStyle=1;align=center;verticalAlign=middle;");
  ...
  protoLayers.forEach((pl, idx) => {
    ...
    cell(`pl_hdr_${idx}`, `<div style="font-size:8px;font-weight:900;color:#1E40AF;text-align:center;">${pl.num}. ${pl.name}</div>`, plx, 102, 152, 16, "text;html=1;strokeColor=none;fillColor=none;align=center;verticalAlign=middle;");

    cell(`pl_desc_${idx}`, `<div style="text-align:center;"><span style="font-size:16px;">${pl.icon}</span><div style="font-size:8px;font-weight:800;color:#0F172A;line-height:1.15;margin-top:2px;">${pl.desc}</div></div>`, plx + 4, 120, 144, 46, "rounded=1;arcSize=4;fillColor=#F8FAFC;strokeColor=#E2E8F0;html=1;align=center;verticalAlign=middle;padding=2;");

    // Protocols Sub-Box
    cell(`pl_proto_${idx}`, `<div style="font-size:8px;color:#1E40AF;font-weight:900;text-align:center;">Protocols</div><div style="font-size:7.5px;color:#0F172A;text-align:center;line-height:1.15;margin-top:2px;">${pl.proto}</div>`, plx + 4, 170, 144, 40, "rounded=1;arcSize=4;fillColor=#EFF6FF;strokeColor=#BFDBFE;html=1;align=center;verticalAlign=middle;padding=2;");

    // Format Sub-Box
    cell(`pl_format_${idx}`, `<div style="font-size:8px;color:#166534;font-weight:900;text-align:center;">Format</div><div style="font-size:7.5px;color:#0F172A;text-align:center;line-height:1.15;margin-top:2px;">${pl.format}</div>`, plx + 4, 214, 144, 40, "rounded=1;arcSize=4;fillColor=#F0FDF4;strokeColor=#BBF7D0;html=1;align=center;verticalAlign=middle;padding=2;");
  ...
  const toolExHtml = `<div style="font-size:8px;line-height:1.3;color:#0F172A;padding:2px 8px;">
  ...
  techToolItems.forEach((tt, idx) => {
    ...
    const gx = 708 + col * 68;
    const gy = 626 + row * 92;
    cell(`tt_${idx}`, `<div style="font-size:14px;text-align:center;">${tt.icon}</div><div style="font-size:7.5px;font-weight:800;color:#0F172A;text-align:center;line-height:1.1;margin-top:2px;">${tt.t}</div>`, gx, gy, 64, 82, "rounded=1;arcSize=4;fillColor=#F8FAFC;strokeColor=#E2E8F0;html=1;align=center;verticalAlign=middle;padding=2;");
  });
  ...
  const bNotesHtml = `<div style="font-size:8px;line-height:1.4;color:#0F172A;padding:2px 6px;">
  ...
  const bAbbrHtml = `<div style="font-size:8px;line-height:1.4;color:#0F172A;padding:2px 8px;">
