...
// Native mxGraph XML matching the approved PNG; every label is XML-safe and editable.
...
function cell(id: string, value: string, style: string, x: number, y: number, w: number, h: number): string {
  return `<mxCell id="${id}" value="${xmlAttr(value)}" style="${style}" vertex="1" parent="1"><mxGeometry x="${x}" y="${y}" width="${w}" height="${h}" as="geometry"/></mxCell>`;
}
...
export function getApprovedSecureDeploymentBlueprintXml(): string {
  ...
  for (const c of cols) {
    ...
    n.push(cell(`${c.id}_title`, `<b>${c.title}</b><br><span style="font-size:10px;color:#43516A">${c.sub}</span>`, `${text}fontSize=12.5;fontColor=#13213B;fontStyle=1;`, c.x+58, 102, c.w-70, 52));
  }
  ...
  card('dev','Developer + Gemini Code Assist',['IDE assistance does not bypass reviews','branch protection or security gates.'],32,168,220,82,'#82AEEF');
  ...
  card('precommit','Pre-commit security',['SAST · Secrets · IaC policy','Dependency scan · License compliance'],32,366,220,78,'#82AEEF');
  ...
  card('evidence','Change evidence',['Commit / PR · reviewer · issue/change reference','provenance status here'],32,566,220,90,'#82AEEF');
  ...
  card('tests','Quality & security tests',['Unit/Integration · SAST · Dependency (SBOM)','Container / IaC scan'],315,268,240,90,'#66B58A');
  card('prov','Build provenance & SBOM',['Generate verifiable provenance','Attach policy-relevant attestations to Artifact Registry'],315,376,240,94,'#66B58A');
  ...
  card('telemetry','Build telemetry',['Build duration · failure reason · scanner findings','provenance generation · audit logs'],315,608,240,78,'#66B58A');
  ...
  card('sign','Signing / provenance',['Sign artifact; verify attestations','against deployment policy'],620,264,255,78,'#E29A43');
  card('binary','Binary Authorization',['Admission policy checks trusted signer/provenance','for supported GKE / Cloud Run runtimes.'],620,358,255,92,'#E29A43');
  card('vuln','Vulnerability decision',['Severity/vulnerability policy determines block,','exception with approval, or promotion.'],620,466,255,82,'#E29A43');
  card('exception','Exception evidence',['Formal approval · expiry · justification','compensating controls · audit trail'],620,566,255,90,'#E29A43');
  ...
  card('nonprod','Non-production',['Deploy immutable artifact · smoke integration tests','policy validation · no rebuild per environment'],940,268,230,84,'#956ED2');
  ...
  card('rollout','Progressive rollout',['Canary · green/blue · traffic shift','deployment strategy appropriate to runtime'],940,470,230,82,'#956ED2');
  card('rollback','Rollback',['Reinstate known-good immutable revision/image','rollback trigger via workload-defined health/SLO breach'],940,586,230,88,'#956ED2');
  ...
  card('private','Private service access',['Private Service Connect for supported producer/consumer endpoints','no generic service-to-service routing'],1234,420,278,88,'#6C91BF');
  ...
  card('ops','Google Security Operations',['Central SIEM/SOAR integration for relevant security telemetry'],1377,640,135,74,'#6C91BF');
  ...
  flow.forEach((label,i)=>{
    n.push(cell(`flow_${i}`,`<b>${label}</b>`,`${box}fillColor=#FFFFFF;strokeColor=#7EA4D6;fontColor=#14376B;fontSize=11.5;align=center;`,fx,938,125,46));
    if(i<flow.length-1) n.push(cell(`flow_arrow_${i}`,'<b>▶</b>',`${text}fontSize=22;fontColor=#14376B;align=center;`,fx+127,942,30,38));
    fx += 170;
  });
