...
export function generateAll16LivingSpecs(ast: ArchitectureAst): LivingSpecDocument[] {
  ...
  return [
    // DOC-01: PRD
    {
      ...
      embeddedFigure: {
        id: "Figure 1.1",
        title: "Global Ingestion & System Context Flow",
        description: "End-to-end user request routing from edge clients to core processing engine.",
        diagramType: "context"
      },
      ...
      markdownContent: [
        ...
        "",
        "> [!NOTE]",
        "> This document serves as the authoritative product specification for **" + meta.projectTitle + "**, defining multi-region scale, latency boundaries, and core business KPIs.",
        "",
        "## 1.0 Executive Problem Statement & Market Context",
        "Enterprise mission-critical workloads require continuous sub-50ms processing across distributed global zones with strict **" + meta.slaTarget + " availability** and zero data loss (**RPO = " + meta.targetRpo + "**). The platform consolidates high-throughput API ingestion, real-time event streaming, and AI-grounded inference into an integrated reference architecture.",
        "",
        "## 2.0 Key Performance Indicators (KPIs)",
        "| Metric Category | Target KPI | Verification Mechanism | Incident Severity |",
        "| :--- | :--- | :--- | :--- |",
        "| **System Availability** | **" + meta.slaTarget + "** (< 5.26 mins/yr downtime) | Multi-Region Active-Active Dual-Hub | P1 Executive Escalation |",
        ...
        "* **FR-101 (Idempotent Execution)**: Every transaction payload must supply an idempotent UUIDv4 token cached in Redis 7.2 for 86,400s to eliminate duplicate processing.",
        "* **FR-102 (Zero-Cleartext Ingress)**: Sensitive customer data and credentials must be tokenized at edge ingress via Cloud KMS HSM; cleartext PII must never touch application memory.",
        "* **FR-103 (Sub-20ms AI Grounding)**: Transaction payloads must be vectorized into 768-dimensional embeddings and matched against historical baselines in under 20ms.",
        "* **FR-104 (Distributed ACID Consistency)**: State changes must execute as two-phase ACID commits in Cloud Spanner, with continuous Change-Data-Capture (CDC) streamed to BigQuery.",
        "",
        "---"
      ].join("\n")
    },
