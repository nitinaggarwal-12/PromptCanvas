...
export const ARCHITECTURE_METADATA_MAP: Record<string, ArchitectureMeta> = {
  ...
  hub_and_spoke_agent_config: {
    ...
    businessUseCase: "Enterprise Hub-and-Spoke Multi-Agent Configuration Map within Gemini Enterprise and Agent Designer. Orchestrator Parent Agent Hub manages General Configuration (Gemini 3.6 Pro, Global System Instructions, Shared Context Window, Memory TTL), Multi-Agent Router / Dispatcher Logic with intent-based rule routing, Shared Memory & State, Vertex AI Agent Runtime, and 21 CFR Part 11 Compliance Gate. Dispatches to 3 specialized Sub-Agents (Customer Support with Zendesk API & BigQuery/Vector Grounding, Fulfillment SA with SAP ERP API & Cloud Storage, and Knowledge SA with GCS PDF/Doc Grounding). Features Logical UI Configuration Matrix (Prompt editors, Rule editors, Knowledge Source selectors, API Config panels), Human-in-the-Loop (HITL) Gate, 21 CFR Part 11 Immutable Audit Trail & E-Signature Ledger, and Persona Dashboards (AI Architect, Solutions Architect, Agent Economic & Runtime Metrics).",
