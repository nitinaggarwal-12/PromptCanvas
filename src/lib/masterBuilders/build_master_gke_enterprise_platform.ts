...
const ICON = {
  ...
  gitops: svg('<circle cx="16" cy="16" r="6" fill="#7C3AED"/><circle cx="32" cy="32" r="6" fill="#7C3AED"/><path d="M16 16v16h16" fill="none" stroke="#7C3AED" stroke-width="3"/>'),
  dns: svg('<rect x="8" y="8" width="32" height="32" rx="6" fill="#2563EB"/><path d="M14 16h20M14 24h20M14 32h14" stroke="#fff" stroke-width="2.5"/>'),
  mci: svg('<rect x="8" y="10" width="32" height="12" rx="3" fill="#2563EB"/><circle cx="14" cy="36" r="5" fill="#2563EB"/><circle cx="34" cy="36" r="5" fill="#2563EB"/><path d="M14 22v9m20-9v9" stroke="#2563EB" stroke-width="2.5"/>'),
  ...
  auth: svg('<circle cx="24" cy="15" r="8" fill="#059669"/><path d="M9 41c2-11 8-16 15-16s13 5 15 16" fill="#059669"/>'),
  ...
  monitor: svg('<rect x="6" y="8" width="36" height="26" rx="3" fill="#EA580C"/><path d="m11 28 7-7 5 4 8-11 6 6" fill="none" stroke="#fff" stroke-width="2.5"/>'),
...
export function buildMasterGkeEnterprisePlatformXml(): string {
  const cells: string[] = [
    ...
    lane('control', 1, 'INGRESS & FLEET CONTROL PLANE', 'Global traffic routing, DNS, Multi-Cluster Ingress & GitOps', 20, 20, 1540, 140, '#1E3A8A', '#FFFFFF'),
    card('ctl_dns', 'Cloud DNS Anycast', 'Global low-latency DNS resolution & DNSSEC', ICON.dns, 45, 75, 270, 70, '#2563EB'),
    card('ctl_mci', 'Global Multi-Cluster Ingress (MCI)', 'Cross-cluster active-active HTTP(S) load balancing', ICON.mci, 345, 75, 340, 70, '#2563EB'),
    card('ctl_gitops', 'Config Sync GitOps Repository', 'Declarative multi-cluster policy & config synchronization', ICON.gitops, 715, 75, 380, 70, '#7C3AED'),
    card('ctl_fleet', 'GKE Enterprise Fleet Manager', 'Unified cluster membership & service mesh management', ICON.gke, 1125, 75, 410, 70, '#1D4ED8'),

    // Lane 2: Multi-Cluster GKE Fleet
    lane('fleet', 2, 'MULTI-CLUSTER GKE FLEET (GKE ENTERPRISE)', 'Cross-region resilient container platform with Cloud Service Mesh', 20, 180, 1540, 310, '#1D4ED8', '#FFFFFF'),
    
    // Cluster A Container (us-central1)
    cell('cl_a_box', '', 'rounded=1;arcSize=6;whiteSpace=wrap;html=1;fillColor=#F8FAFC;strokeColor=#CBD5E1;strokeWidth=1.5;', 45, 235, 710, 235),
    cell('cl_a_title', '<b>Primary GKE Enterprise Cluster A (us-central1)</b>', 'text;html=1;whiteSpace=wrap;align=left;verticalAlign=middle;fontColor=#1D4ED8;fontSize=13;fontStyle=1;', 65, 245, 400, 25),
    card('cl_a_mesh', 'Cloud Service Mesh (mTLS 1.3)', 'Istio / Envoy proxy sidecars with strict mTLS', ICON.mesh, 65, 280, 320, 70, '#2563EB'),
    card('cl_a_cilium', 'Cilium eBPF CNI Policies', 'Kernel-level microsegmentation & DDoS defense', ICON.shield, 410, 280, 320, 70, '#059669'),
    card('cl_a_wif', 'Workload Identity Federation', 'KSA to GSA short-lived token impersonation', ICON.auth, 65, 365, 320, 70, '#059669'),
    ...
    cell('cl_b_box', '', 'rounded=1;arcSize=6;whiteSpace=wrap;html=1;fillColor=#F8FAFC;strokeColor=#CBD5E1;strokeWidth=1.5;', 785, 235, 750, 235),
    cell('cl_b_title', '<b>Secondary GKE Enterprise Cluster B (us-east4)</b>', 'text;html=1;whiteSpace=wrap;align=left;verticalAlign=middle;fontColor=#1D4ED8;fontSize=13;fontStyle=1;', 805, 245, 400, 25),
    card('cl_b_mesh', 'Cloud Service Mesh (mTLS 1.3)', 'Cross-region peer mesh service discovery', ICON.mesh, 805, 280, 340, 70, '#2563EB'),
    card('cl_b_cilium', 'Cilium eBPF CNI Policies', 'Kernel-level microsegmentation & DDoS defense', ICON.shield, 1170, 280, 340, 70, '#059669'),
    card('cl_b_wif', 'Workload Identity Federation', 'KSA to GSA short-lived token impersonation', ICON.auth, 805, 365, 340, 70, '#059669'),
    ...
    lane('platform', 3, 'PLATFORM SERVICES & PERSISTENCE TIER', 'Enterprise storage, caching, secret management and monitoring', 20, 510, 1540, 190, '#D97706', '#FFFFFF'),
    card('plt_prom', 'Managed Prometheus', 'Global metrics aggregation & PromQL dashboards', ICON.monitor, 45, 575, 280, 95, '#EA580C'),
    card('plt_log', 'Cloud Logging & Tracing', 'Cloud Trace latency profiling & centralized audit logs', ICON.log, 345, 575, 280, 95, '#059669'),
    card('plt_sec', 'Secret Manager & KMS', 'Dynamic secret injection via CSI Driver & CMEK', ICON.key, 645, 575, 280, 95, '#D97706'),
    ...
    lane('security', 4, 'SECURITY & GOVERNANCE FOUNDATION', 'End-to-end container security, admission control and threat detection', 20, 720, 1540, 190, '#047857', '#FFFFFF'),
    card('sec_binauth', 'Binary Authorization', 'Signature verification & supply chain attestations', ICON.shield, 45, 785, 340, 95, '#059669'),
    card('sec_policy', 'Policy Controller (Gatekeeper)', 'OPA-based declarative admission webhooks', ICON.shield, 415, 785, 340, 95, '#059669'),
    card('sec_scc', 'Security Command Center (SCC)', 'Container threat detection & posture management', ICON.shield, 785, 785, 360, 95, '#059669'),
    card('sec_vuln', 'Artifact Analysis Scanner', 'Automated container image CVE scanning', ICON.shield, 1175, 785, 360, 95, '#059669'),
  ...
  return `<mxfile host="app.diagrams.net" modified="2026-08-21T00:35:00.000Z" agent="PromptCanvas Blueprint 52" version="24.7.17" type="device">
