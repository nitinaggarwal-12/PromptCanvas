 ...
 * Polished enterprise orientation view with vendor ecosystem, current Google Cloud
 * services, Gemini Enterprise capabilities, and explicit cross-cutting controls.
 */
...
const ICONS = {
  ...
  gke: 'https://cdn.jsdelivr.net/gh/glincker/thesvg@main/public/icons/google-kubernetes-engine/default.svg',
};
...
const card = (
  ...
  [
    ...
    image(`${id}_icon`, iconUrl, x + 14, y + 22, 38, 38),
    v(`${id}_label`, `<b>${title}</b><br><span style="font-size:9.5px;color:#64748B">${subtitle}</span>`, 'text;html=1;whiteSpace=wrap;overflow=hidden;spacing=4;spacingLeft=3;spacingRight=3;align=left;verticalAlign=middle;fontColor=#0F172A;fontSize=11.5;', x + 62, y + 7, width - 74, 72),
  ].join('\n');
...
const chip = (
  ...
  v(id, label, `rounded=1;arcSize=16;whiteSpace=wrap;html=1;overflow=hidden;spacing=3;fillColor=${fill};strokeColor=${accent};strokeWidth=1;fontColor=#334155;fontSize=9.5;fontStyle=1;align=center;verticalAlign=middle;`, x, y, width, 24);
...
const cross = (
  ...
  [
    ...
    v(`${id}_badge`, '', `ellipse;whiteSpace=wrap;html=1;fillColor=${accent};strokeColor=${accent};`, 1400, y + 29, 42, 42),
    image(`${id}_mark`, GCP_LOGO, 1407, y + 36, 28, 28),
    v(`${id}_label`, `<b>${title}</b><br><span style="font-size:9.5px;color:#64748B">${subtitle}</span>`, 'text;html=1;whiteSpace=wrap;overflow=hidden;spacing=4;align=left;verticalAlign=middle;fontColor=#0F172A;fontSize=11.5;', 1454, y + 10, 250, 80),
  ].join('\n');
...
export function buildEnterpriseReferenceArchitectureXml(): string {
  const cells: string[] = [
    ...
    // Experience: personas and channels, intentionally technology-light.
    ...
    // Enterprise systems: recognizable vendor ecosystem, not anonymous ERP boxes.
    ...
    // AI & Agentic: explicit Gemini Enterprise capabilities with correct boundaries.
    v('ai_ge', '', 'rounded=1;arcSize=8;whiteSpace=wrap;html=1;overflow=hidden;fillColor=#FFFFFF;strokeColor=#B83280;strokeWidth=1.3;', 280, 590, 488, 90),
    image('ai_ge_icon', GCP_LOGO, 296, 606, 42, 42),
    v('ai_ge_title', '<b>Gemini Enterprise</b><br><span style="font-size:9.5px;color:#64748B">Employee AI hub for enterprise search, assistant experiences and governed agents</span>', 'text;html=1;whiteSpace=wrap;overflow=hidden;spacing=3;align=left;verticalAlign=middle;fontColor=#0F172A;fontSize=12;', 350, 595, 400, 40),
    chip('ai_ge_connectors', 'Connectors', 350, 642, 88, '#B83280', '#FDF2F8'),
    chip('ai_ge_notebook', 'Gemini Notebook', 444, 642, 108, '#B83280', '#FDF2F8'),
    chip('ai_ge_skills', 'Skills', 558, 642, 70, '#B83280', '#FDF2F8'),
    chip('ai_ge_agents', 'Agent Gallery / Designer', 634, 642, 116, '#B83280', '#FDF2F8'),

    v('ai_platform', '', 'rounded=1;arcSize=8;whiteSpace=wrap;html=1;overflow=hidden;fillColor=#FFFFFF;strokeColor=#B83280;strokeWidth=1.3;', 780, 590, 296, 90),
    image('ai_platform_icon', GCP_LOGO, 796, 607, 40, 40),
    v('ai_platform_label', '<b>Gemini Enterprise Agent Platform</b><br><span style="font-size:9.5px;color:#64748B">Agent Studio / ADK • Agent Engine • tool orchestration • custom agents</span>', 'text;html=1;whiteSpace=wrap;overflow=hidden;spacing=4;align=left;verticalAlign=middle;fontColor=#0F172A;fontSize=11.5;', 848, 597, 212, 72),

    v('ai_trust', '', 'rounded=1;arcSize=8;whiteSpace=wrap;html=1;overflow=hidden;fillColor=#FFFFFF;strokeColor=#B83280;strokeWidth=1.3;', 1088, 590, 208, 90),
    image('ai_trust_icon', GCP_LOGO, 1104, 608, 38, 38),
    v('ai_trust_label', '<b>AI Trust & Quality</b><br><span style="font-size:9.5px;color:#64748B">Grounding • evaluation • Model Armor • human authority where required</span>', 'text;html=1;whiteSpace=wrap;overflow=hidden;spacing=3;align=left;verticalAlign=middle;fontColor=#0F172A;fontSize=11;', 1152, 597, 128, 72),
    ...
    cross('cross_identity', 'Identity & Access', 'IAM • Workforce Identity Federation • Workload Identity Federation • least privilege', 96, '#1A73E8'),
    cross('cross_security', 'Security & AI Safety', 'Security Command Center • Cloud Armor • KMS • Secret Manager • Model Armor where applicable', 208, '#D93025'),
    cross('cross_governance', 'Governance & Audit', 'Org policy • VPC Service Controls • audit logs • data/AI policy • human approvals', 320, '#7B61A8'),
    cross('cross_observability', 'Observability & SRE', 'Cloud Monitoring • Logging • Trace • SLOs • alerting • incident response', 432, '#0F8B82'),
    cross('cross_delivery', 'DevSecOps & IaC', 'GitHub or Secure Source Manager • Cloud Build • Artifact Registry • Terraform', 544, '#E87900'),
    cross('cross_finops', 'FinOps & Capacity', 'Budgets • attribution • quotas • scaling • performance and cost optimization', 656, '#C77B00'),

    // Orientation spine: explicit end-to-end direction without crossing content.
    flow('f12', 'experience_n', 'business_n'),
    flow('f23', 'business_n', 'integration_n'),
    flow('f34', 'integration_n', 'data_n'),
    flow('f45', 'data_n', 'ai_n'),
    flow('f56', 'ai_n', 'platform_n'),

    // Footer: scope, ecosystem and semantic legend.
    ...
    image('footer_gcp_logo', GCP_LOGO, 50, 888, 50, 50),
    v('footer_gcp', '<b>Google Cloud foundation</b><br><span style="font-size:9.5px;color:#64748B">Specialized blueprints provide implementation-level topology for network, IAM, DR, MCP, data, SRE, CI/CD and threat modeling.</span>', 'text;html=1;whiteSpace=wrap;overflow=hidden;spacing=4;align=left;verticalAlign=middle;fontColor=#0F172A;fontSize=12;', 112, 878, 400, 70),

    image('footer_ms', ICONS.microsoft, 548, 886, 32, 32),
    image('footer_sf', ICONS.salesforce, 590, 886, 38, 32),
    image('footer_sap', ICONS.sap, 640, 886, 44, 32),
    image('footer_sn', ICONS.servicenow, 696, 886, 38, 32),
    image('footer_gh', ICONS.github, 746, 886, 32, 32),
    v('footer_ecosystem', '<b>Enterprise ecosystem</b><br><span style="font-size:9.5px;color:#64748B">Connect existing systems; do not imply forced migration or a single-vendor estate.</span>', 'text;html=1;whiteSpace=wrap;overflow=hidden;spacing=4;align=left;verticalAlign=middle;fontColor=#0F172A;fontSize=11.5;', 548, 925, 280, 46),

    v('footer_ge_note', '<b>Gemini Enterprise capability boundary</b><br><span style="font-size:9.5px;color:#64748B">Connectors ground enterprise access. Gemini Notebook supports research/knowledge workflows. Skills are reusable assistant instructions and are not agent workflows. Agent Gallery / Agent Designer cover no-code agent use; Agent Platform / ADK serve custom agent engineering.</span>', 'rounded=1;arcSize=7;whiteSpace=wrap;html=1;overflow=hidden;spacing=6;fillColor=#FDF2F8;strokeColor=#D89BC0;strokeWidth=1;fontColor=#334155;fontSize=10.5;align=left;verticalAlign=middle;', 850, 878, 520, 92),

    v('footer_legend', '<b>VIEW SEMANTICS</b><br><span style="font-size:9.5px;color:#64748B">Numbered rail = enterprise orientation flow<br>Colored layers = architecture responsibility boundaries<br>Right rail = cross-cutting controls, not a separate runtime path</span>', 'rounded=1;arcSize=7;whiteSpace=wrap;html=1;overflow=hidden;spacing=7;fillColor=#F8FAFC;strokeColor=#CBD5E1;strokeWidth=1;fontColor=#334155;fontSize=10.5;align=left;verticalAlign=middle;', 1390, 878, 330, 92),

    v('scope_note', '<b>REFERENCE VIEW — NOT A DEPLOYMENT TOPOLOGY:</b> This blueprint orients executives and architects across enterprise layers. Use the specialized catalog blueprints for detailed product wiring, security boundaries, network paths, failover behavior, protocols and operational runbooks.', 'text;html=1;whiteSpace=wrap;overflow=hidden;spacing=4;align=center;verticalAlign=middle;fontColor=#475569;fontSize=10.5;', 50, 982, 1670, 30),
  ];
