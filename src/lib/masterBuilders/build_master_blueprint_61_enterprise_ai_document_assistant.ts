 ...
 * Executive Light Theme Master Architecture Blueprint
 * 
 * Featuring:
...
export function buildMasterBlueprint61EnterpriseAiDocumentAssistantXml(): string {
  const cells: string[] = [
    ...
    cell('title', 'Blueprint 61 — Enterprise AI Document Assistant Platform — Detailed Architecture & Flowchart', 'text;html=1;whiteSpace=wrap;fontColor=#0F172A;fontSize=18;fontStyle=1;align=left;verticalAlign=middle;', 18, 12, 1280, 24),
    cell('subtitle', 'Secure Ingestion, RAG, Multi-Agent Orchestration, Human-in-the-Loop Review, and Observability on Google Cloud', 'text;html=1;whiteSpace=wrap;fontColor=#2563EB;fontSize=11.5;fontStyle=1;align=left;verticalAlign=middle;', 18, 36, 1280, 18),

    // =========================================================================
    // TIER 1: USERS & CHANNELS
    // =========================================================================
    lane('t1', 'TIER 1', 'USERS &amp;<br/>CHANNELS', 18, 65, 1260, 95, '#CBD5E1', '#FFFFFF', '#1E293B'),
    pill('u_biz', 'Business User', 'Web Browser', ICON.user, 145, 82, 130, 60, '#2563EB'),
    pill('u_analyst', 'Knowledge Analyst', 'Copilot UI', ICON.analyst, 285, 82, 130, 60, '#059669'),
    pill('u_admin', 'System Admin', 'Admin Console', ICON.admin, 425, 82, 130, 60, '#DC2626'),
    pill('ch_web', 'Web Client', 'React / Next.js', ICON.user, 575, 82, 125, 60, '#2563EB'),
    pill('ch_mob', 'Mobile App', 'iOS / Android SDK', ICON.user, 710, 82, 125, 60, '#2563EB'),
    pill('ch_teams', 'MS Teams / Slack', 'Enterprise Bot', ICON.user, 845, 82, 130, 60, '#7C3AED'),
    pill('ch_api', 'B2B API Client', 'REST / OpenAPI', ICON.apigw, 985, 82, 125, 60, '#2563EB'),
    pill('ch_resp', 'Unified Response', 'Streaming Citations', ICON.gemini, 1120, 82, 140, 60, '#059669'),

    // =========================================================================
    // TIER 2: EDGE, IDENTITY & SECURITY GATEWAY
    // =========================================================================
    lane('t2', 'TIER 2', 'EDGE DEFENSE<br/>&amp; SECURITY', 18, 170, 1260, 100, '#CBD5E1', '#FFFFFF', '#1E3A8A'),
    card('n_gclb', 'External HTTPS LB', 'Global Anycast IP • TLS 1.3', ICON.gclb, 145, 185, 180, 70, '#2563EB'),
    card('n_armor', 'Cloud Armor WAF', 'Layer 7 DDoS • OWASP Top 10', ICON.armor, 335, 185, 180, 70, '#2563EB'),
    card('n_apigw', 'API Gateway', 'Quota Limiting • JWT Validate', ICON.apigw, 525, 185, 175, 70, '#2563EB'),
    card('n_idp', 'Identity Platform &amp; SSO', 'OIDC • OAuth 2.0 • MFA Gates', ICON.idp, 710, 185, 185, 70, '#7C3AED'),
    decision('d_auth', 'Valid Auth &amp;<br/>Role Token?', 905, 182, 110, 75, '#D97706', '#FEF3C7'),
    card('n_sec', 'Secret Manager', 'Automatic API Key Rotation', ICON.secrets, 1025, 185, 115, 70, '#D97706'),
    card('n_kms', 'Cloud KMS (CMEK)', 'Hardware Envelope Crypto', ICON.kms, 1150, 185, 115, 70, '#059669'),

    // =========================================================================
    // TIER 3: APPLICATION & MULTI-AGENT ORCHESTRATION
    // =========================================================================
    lane('t3', 'TIER 3', 'APPLICATION &amp;<br/>ORCHESTRATION', 18, 280, 1260, 120, '#CBD5E1', '#FFFFFF', '#4C1D95'),
    card('n_bff', 'UI Backend / BFF', 'Cloud Run • Session Adapter', ICON.gke, 145, 300, 155, 80, '#7C3AED'),
    card('n_orch', 'Workflow Orchestrator', 'Multi-Agent State &amp; Retries', ICON.gke, 310, 300, 160, 80, '#7C3AED'),
    card('n_val', 'Document Processing', 'MIME &amp; Size Check', ICON.docai, 480, 300, 145, 80, '#2563EB'),
    decision('d_doc_type', 'Document Type<br/>Supported?', 635, 302, 100, 75, '#D97706', '#FEF3C7'),
    card('n_agent_hub', 'Supervisor Agent', 'Goal Planning &amp; Tools', ICON.gemini, 745, 300, 155, 80, '#7C3AED'),
    decision('d_conf', 'Confidence &gt;=<br/>Threshold?', 910, 302, 100, 75, '#D97706', '#FEF3C7'),
    card('n_hitl', 'Human Review Task', 'Approve / Edit / Reject', ICON.admin, 1020, 300, 120, 80, '#DC2626'),
    card('n_notif', 'Notification Service', 'Eventarc • Alert Triggers', ICON.pubsub, 1150, 300, 115, 80, '#059669'),

    // =========================================================================
    // TIER 4: AI / ML & AGENTIC SERVICES
    // =========================================================================
    lane('t4', 'TIER 4', 'AI / ML SERVICES<br/>&amp; GEMINI', 18, 410, 1260, 120, '#CBD5E1', '#FFFFFF', '#065F46'),
    card('n_docai', 'Document AI &amp; OCR', 'Layout Parser • Tables • Entities', ICON.docai, 145, 430, 165, 80, '#0284C7'),
    card('n_emb', 'Multilingual Embedding', 'text-embedding-004', ICON.gemini, 320, 430, 150, 80, '#0284C7'),
    card('n_rerank', 'Vertex AI Reranker', 'Cross-Encoder Re-scoring', ICON.vector, 480, 430, 150, 80, '#0284C7'),
    card('n_gemini', 'Vertex AI Gemini 2.5', 'Multi-Step Reasoning &amp; RAG', ICON.gemini, 640, 430, 165, 80, '#7C3AED'),
    card('n_mcp_tools', 'Agent Tools &amp; MCP', 'Dynamic Function Calling API', ICON.tools, 815, 430, 165, 80, '#D97706'),
    decision('d_grounded', 'Grounded Context<br/>Verified?', 990, 432, 100, 75, '#D97706', '#FEF3C7'),
    card('n_armor_ai', 'Model Armor &amp; Safety', 'PII Mask • Injection Defense', ICON.modelarmor, 1100, 430, 165, 80, '#059669'),

    // =========================================================================
    // TIER 5: DATA SOURCES, VECTOR STORES & KNOWLEDGE MESH
    // =========================================================================
    lane('t5', 'TIER 5', 'DATA STORES &amp;<br/>LAKEHOUSE', 18, 540, 1260, 110, '#CBD5E1', '#FFFFFF', '#1E3A8A'),
    ...
    card('n_vec', 'Vertex Vector Search', 'ScANN Vector Store &amp; Index', ICON.vector, 510, 555, 175, 75, '#0284C7'),
    card('n_sql', 'Cloud SQL / AlloyDB', 'App &amp; Review Workflow State', ICON.sql, 695, 555, 175, 75, '#2563EB'),
    card('n_fire', 'Firestore Session Store', 'Fast Turn-by-Turn Memory', ICON.fire, 880, 555, 160, 75, '#D97706'),
    card('n_ext', 'Enterprise Connectors', 'SharePoint • Salesforce • SAP', ICON.tools, 1050, 555, 215, 75, '#7C3AED'),

    // =========================================================================
    // TIER 6: EVENTING & INTEGRATION HUB
    // =========================================================================
    lane('t6', 'TIER 6', 'EVENTING &amp;<br/>INTEGRATION', 18, 660, 1260, 95, '#CBD5E1', '#FFFFFF', '#9D174D'),
    card('n_pubsub', 'Cloud Pub/Sub', 'Asynchronous Intake Topics', ICON.pubsub, 145, 675, 200, 65, '#DB2777'),
    card('n_eventarc', 'Eventarc Triggers', 'Audit &amp; Mutation Event Router', ICON.pubsub, 360, 675, 200, 65, '#DB2777'),
    card('n_workers', 'Cloud Run / Functions', 'Serverless Async Ingestion Workers', ICON.fn, 575, 675, 220, 65, '#DB2777'),
    card('n_webhooks', 'Webhooks / Outbound', 'Third-Party Push &amp; Sync APIs', ICON.apigw, 810, 675, 200, 65, '#DB2777'),
    card('n_integrations', 'Integration Connectors', 'Enterprise ERP &amp; CRM Sync', ICON.tools, 1025, 675, 240, 65, '#DB2777'),

    // =========================================================================
    // TIER 7: PLATFORM DEVOPS & SECURITY GOVERNANCE
    // =========================================================================
    lane('t7', 'TIER 7', 'DEVOPS, SRE &amp;<br/>GOVERNANCE', 18, 765, 1260, 95, '#CBD5E1', '#FFFFFF', '#065F46'),
    card('n_cicd', 'GitHub &amp; Cloud Build', 'Polyrepo CI/CD • Artifact Registry', ICON.gke, 145, 780, 200, 65, '#2563EB'),
    card('n_iac', 'Terraform IaC', 'Declarative Module Automation', ICON.iac, 360, 780, 175, 65, '#7C3AED'),
    card('n_logging', 'Cloud Logging &amp; SRE', 'Error Reporting &amp; Monitoring', ICON.log, 550, 780, 200, 65, '#059669'),
    card('n_iam_gov', 'IAM &amp; Least Privilege', 'Zero-Trust RBAC &amp; Service Accounts', ICON.admin, 765, 780, 185, 65, '#2563EB'),
    card('n_dlp', 'Sensitive Data DLP', 'Automatic PII Redaction Engine', ICON.modelarmor, 965, 780, 150, 65, '#059669'),
    card('n_audit', 'Cloud Audit Logs', 'Immutable Compliance Evidence', ICON.log, 1130, 780, 135, 65, '#059669'),

    // =========================================================================
    // SIDE PANELS (LEGEND, FALLBACKS, SECURITY & STEP KEY)
    // =========================================================================
    cell('p_legend', `<div style="padding:10px;"><b style="font-size:11px;color:#1E3A8A;">SYSTEM LEGEND</b><br/><div style="margin-top:6px;font-size:9.5px;color:#475569;line-height:1.5;">— <b>Blue Line:</b> Synchronous HTTPS Flow<br/>- - <b>Dashed Line:</b> Data &amp; Vector Pipeline<br/>— <b>Green Line:</b> Verified Response / Commit<br/>◇ <b>Amber Diamond:</b> Decision Policy Gate</div></div>`, 'rounded=1;arcSize=8;whiteSpace=wrap;html=1;fillColor=#F8FAFC;strokeColor=#CBD5E1;strokeWidth=1.2;align=left;verticalAlign=top;', 1290, 65, 270, 110),
    ...
    edge('e_docai_emb', 'n_docai', 'n_emb', 'Generate Vectors', '#0284C7', 0),
    edge('e_emb_vec', 'n_emb', 'n_vec', '6. Store Vector Index', '#0284C7', 1, 0.5, 1, 0.5, 0, [[422, 525], [597, 525]]),
    edge('e_vec_rerank', 'n_vec', 'n_rerank', '7. Candidate Chunks', '#0284C7', 0, 0.5, 0, 0.5, 1),
    edge('e_rerank_agent', 'n_rerank', 'n_agent_hub', 'Grounded Context', '#7C3AED', 0, 0.5, 0, 0.5, 1),
    edge('e_agent_gemini', 'n_agent_hub', 'n_gemini', '8. Plan &amp; Reason', '#7C3AED', 0, 0.5, 1, 0.5, 0),
    edge('e_gemini_tools', 'n_gemini', 'n_mcp_tools', '9. Invoke Tools', '#D97706', 0),
    edge('e_tools_ext', 'n_mcp_tools', 'n_ext', 'Query ERP/CRM', '#D97706', 1, 0.5, 1, 0.5, 0, [[987, 525], [1157, 525]]),
    edge('e_armor_conf', 'n_armor_ai', 'd_conf', 'Evaluate Confidence', '#D97706', 0, 0.5, 0, 0.5, 1, [[1175, 395], [955, 395]]),
    edge('e_conf_hitl', 'd_conf', 'n_hitl', 'Score &lt; 90% (HITL)', '#DC2626', 0),
    edge('e_conf_resp', 'd_conf', 'ch_resp', '11. Stream Response', '#059669', 0, 0.5, 0, 0.5, 1, [[955, 140], [1190, 140]]),
    edge('e_hitl_notif', 'n_hitl', 'n_notif', 'Send Review Alert', '#DC2626', 0),
    edge('e_notif_teams', 'n_notif', 'ch_teams', 'Teams/Slack Push', '#DC2626', 0, 0.5, 0, 0.5, 1, [[1207, 280], [910, 280]]),
    edge('e_docai_bq', 'n_docai', 'n_bq', 'Extract Line Items', '#1D4ED8', 1, 0.5, 1, 0.5, 0, [[235, 525], [412, 525]]),
    edge('e_fire_bff', 'n_fire', 'n_bff', 'Restore State', '#D97706', 1, 0.5, 0, 0.5, 0, [[960, 640], [130, 640], [130, 340]]),
    edge('e_pub_eventarc', 'n_pubsub', 'n_eventarc', 'Event Fan-out', '#DB2777', 0),
    edge('e_eventarc_workers', 'n_eventarc', 'n_workers', 'Trigger Workers', '#DB2777', 0),
    edge('e_workers_webhooks', 'n_workers', 'n_webhooks', 'Outbound Push', '#DB2777', 0),
