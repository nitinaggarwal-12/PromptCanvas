...
39. **Google Docs (`.docx`) 100% Native Editable Draw.io Architecture Diagram Canvas & 1-Click Live Draw.io Web Editor Law (`exportDrawioToEditableDocx`)**:
    - Whenever compiling `.docx` specifications for Google Docs (`exportDrawioToEditableDocx` and `/api/export/cloud-bridge/[filename]/route.ts`), the generated document MUST NEVER rely solely on a flat static PNG bitmap image or instruct the user to switch to Google Slides `.pptx` to edit diagram boxes.
    - Every `.docx` generated by `exportDrawioToEditableDocx` MUST contain:
      1. **1-Click Live Draw.io Web Editor Action Bar (`ExternalHyperlink`)**: Top clickable action bar with direct hyperlinks to launch the full `.drawio` XML inside `https://app.diagrams.net/?title=...#U...drawio` and `/vision?id=...`.
      2. **Section 1.1: Interactive Editable Draw.io Architecture Diagram Canvas (Native Google Docs Editable Vector Boxes & Enclaves)**: Spatially groups all Draw.io vertices into their enclosing Enclave Containers and Service Nodes, rendering a 100% native WordprocessingML (`Table` / `TableCell`) 3-column visual vector card grid with authentic Draw.io pastel fill tints, 3pt colored left borders (`strokeColor`), editable bold Node Titles, editable Subtitles/Attributes, and directional connector flow pills (`━━▶`).
      3. **Companion `.drawio` Cloud Bridge Serving & Schema Auto-Healing**: `/api/export/cloud-bridge/[filename]/route.ts` MUST serve `.drawio` XML files with `Access-Control-Allow-Origin: *` and `Content-Type: application/xml; charset=utf-8`, and check `CURRENT_BRIDGE_SCHEMA_VERSION` (`v3_editable_drawio_canvas`) so stale cached `.docx` files (including `vis5965_*` and `vis9745_*`) automatically self-heal on demand.
